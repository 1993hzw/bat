var init = function () {
    ...
    var visits = 0;//博客访问量
    dbHolder.openDB()
        .then(function (resultDB) {//初始化数据库
            return dbHolder.beginTransaction(resultDB)
                .then(dbHolder.initDB)
                .then(function () {
                    return dbHolder.commitTransaction(resultDB);
                });
        })
        ...
        .then(function () {
            ...
            if (!val) {
                ...
                var data = {};
                var brief;
                var markdown = require('fs').readFileSync(APP_PATH + '/data/guide.md', {encoding: 'utf-8'});
                ...
                data[fields.tags] = 1;//默认
                data[fields.status] = 0; //公开
        ...
        .catch(function (err) {
            ...
            setTimeout(function () {//初始化失败，退出进程
                process.emit('exit', 1);
            }, 1000);
        })
};
